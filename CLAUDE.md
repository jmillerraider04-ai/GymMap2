...
9. **Multi-pin constraint trap — surface contacts must use ONE fixed constraint at the center of pressure, not multiple pins at discrete points.** Placing two fixed constraints on the same bone (e.g., ankle pin at foot position 0, toe pin at position 1) fully locks that bone's pose kinematically, but it over-constrains Phase B: two fixed constraints produce 6 reaction DOFs for what is physically a 3-DOF surface contact, and Phase B's Σ(τ/cap)² minimization then concentrates the reactions at whichever pin has the smallest moment arm for the joints above — often *zero* moment arm, which silently zeroes out demand at every joint between the force and the support. The canonical failure: feet pinned at ankle+toe for a stiff-leg deadlift loads the ankle pin fully (moment arm to hip is zero for a Y-reaction directly below the hip), Phase B reports zero hip demand despite 20 units of gravity needing to be balanced by hip extensors. The physical reality is a pressure distribution integrating to a single net force at the CoP — so use one fixed constraint at position 0.5 (midfoot, or wherever CoP sits for the exercise). Ankle muscles then carry whatever foot-orientation moment the external load demands, which matches reality. Multi-pin on the same bone is valid only when modeling GENUINELY SEPARATE contacts (e.g., BB bench press pins pelvis AND neck to represent two pads of the bench — those are physically distinct surfaces).
